...
fitCopula <- function(X,ll,ul, nChains){
  ...
  if(ns > npoints){
    ...
    cat(sprintf("--Fitting Copula using %i sample points (total sample consists of points=%i)\n", npoints, ns))
  }else{
    I <- 1:ns
    cat(sprintf("--Fitting Copula using %i sample points (sample smaller than max nr Copula points=%i)\n", ns, npoints))
  }
  # add max and min
...
makeIndepCopula_JA <- function(ll, ul){
  #browser()
  npoints <- 1000
  np <- length(ll)
